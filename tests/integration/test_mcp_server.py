...
def test_mcp_server_tools_delegate_to_service(monkeypatch: pytest.MonkeyPatch):
    ...
    results = mcp_server.search_users(query="premium customer", top_k=1)
    listed = mcp_server.list_users(limit=10, offset=0)

    assert created == 101
    assert asdict(found)["email"] == "ana@example.com"
    assert asdict(results[0])["score"] == 0.99
    assert asdict(listed[0])["id"] == 101
    assert stub.calls == [
        (
            "create_user",
            {
        ...
        ("search_users", {"query": "premium customer", "top_k": 1}),
